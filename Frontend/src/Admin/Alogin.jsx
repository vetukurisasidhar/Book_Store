...
const Alogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    try {
      const res = await axios.post(window.BACKEND_URL + '/api/admin/login', formData);
    ...
    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '100vh', padding: '1.5rem' }}>
      <div className="glass-card animate-fade" style={{ width: '100%', maxWidth: '400px' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <button type="submit" className="btn btn-danger" style={{ width: '100%', marginTop: '0.5rem' }}>Log In</button>
        </form>
        <p style={{ marginTop: '1.5rem', textAlign: 'center', color: 'var(--text-secondary)', fontSize: '0.9rem' }}>
          New Admin? <Link to="/admin/signup" style={{ color: 'var(--danger)', fontWeight: 500 }}>Register here</Link>
        </p>
      </div>
    </div>
  );
